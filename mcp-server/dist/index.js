...
function runCtxArgv(args, projectPath, skipAutoInit = false) {
...
function safeRead(filePath, maxChars = 10_000) {
...
function recordActivity(projectPath, tool, summary) {
    try {
        const entry = {
...
function recentActivity(projectPath, count = 5) {
    try {
...
function withRecentActivity(projectPath, baseText, tool, summary) {
...
function compactParagraph(text, maxLen = 260) {
    const cleaned = text
        .replace(/```[\s\S]*?```/g, " ")
        .replace(/[#>*`|]/g, " ")
        .replace(/\s+/g, " ")
...
function buildProjectOverview(projectPath) {
    ...
    const hasWs = combinedLower.includes("websocket") ||
    ...
    const hasAuth = combinedLower.includes("auth") ||
...
// ── Tool: ctx_init ──────────────────────────────────────────────────
server.tool("ctx_init", "Initialize ctx in a project directory. Creates a project-specific database in the global ctx store, scans all files, extracts symbols (functions, classes, structs), maps dependencies, and analyzes git history for decisions.", ProjectPathSchema.shape, async ({ project_path }) => {
...
server.tool("ctx_status", "Get project dashboard: total files, lines of code, symbols, dependencies, decisions, knowledge notes, and language breakdown. Always appends a compact project overview (purpose, users, modules, critical flows). If there are no knowledge notes yet, it automatically saves the first overview note.", ProjectPathSchema.shape, async ({ project_path }) => {
    ...
    const text = withRecentActivity(project_path, `${output}${suffix}`, "ctx_status", "fetch project status dashboard");
    return { content: [{ type: "text", text }] };
});
// ── Tool: ctx_map ───────────────────────────────────────────────────
server.tool("ctx_map", "Display a structured codebase map showing the directory tree with file counts, line counts, and language breakdown per directory. Ideal for understanding project structure at a glance.", ProjectPathSchema.shape, async ({ project_path }) => {
    const { output } = runCtxArgv(["map"], project_path);
    const text = withRecentActivity(project_path, output, "ctx_map", "render codebase map");
    return { content: [{ type: "text", text }] };
});
// ── Tool: ctx_scan ──────────────────────────────────────────────────
server.tool("ctx_scan", "Re-scan the project incrementally. Only analyzes files whose content hash has changed. Updates symbols, dependencies, and the full-text search index.", ProjectPathSchema.shape, async ({ project_path }) => {
    const { output } = runCtxArgv(["scan"], project_path);
    const text = withRecentActivity(project_path, output, "ctx_scan", "incremental re-scan");
    return { content: [{ type: "text", text }] };
});
// ── Tool: ctx_query ─────────────────────────────────────────────────
server.tool("ctx_query", "Full-text search across all symbols (functions, classes, structs, enums, etc.) using FTS5. Returns matching symbols with their full signatures and file locations. Supports partial matching.", {
    ...
    const text = withRecentActivity(project_path, merged, "ctx_query", `symbol query with fallback: ${query}`);
    return { content: [{ type: "text", text }] };
});
...
// ── Tool: ctx_decisions ─────────────────────────────────────────────
server.tool("ctx_decisions", "List all recorded architectural decisions. Includes decisions auto-extracted from conventional commits (feat/fix/refactor/breaking) and manually added entries.", ProjectPathSchema.shape, async ({ project_path }) => {
...
// ── Tool: ctx_learn ─────────────────────────────────────────────────
server.tool("ctx_learn", "Store a knowledge note about the project. Use this to record architectural insights, gotchas, design rationale, or any context that would help future development. Optionally link to a specific file.", {
    ...ProjectPathSchema.shape,
    ...
    const text = withRecentActivity(project_path, output, "ctx_learn", file ? `store knowledge note for ${file}` : "store knowledge note");
    return { content: [{ type: "text", text }] };
});
// ── Tool: ctx_warnings ──────────────────────────────────────────────
server.tool("ctx_warnings", "Show codebase health warnings: fragile files (high churn + many dependents), large files (>500 lines), and potentially dead code (no commits, no dependents). Helps prioritize refactoring.", ProjectPathSchema.shape, async ({ project_path }) => {
...
// ── Tool: ctx_overview ──────────────────────────────────────────────
server.tool("ctx_overview", "Build an agent-ready project overview (purpose, users, modules, critical flows) from repository docs and structure. Also stores this overview as a knowledge note when none exists yet, unless disabled.", {
    ...ProjectPathSchema.shape,
    save_note: z
        .boolean()
        .optional()
    ...
    const finalText = withRecentActivity(project_path, text, "ctx_overview", `generate project overview (save_note=${save_note !== false})`);
...
server.tool("ctx_guard", "Run paranoid security guard checks. If auth/session/token/crypto-related files are touched, this gate can return BLOCK unless critical controls are present (rotation, replay detection, global revoke, rate limiting, and tests).", ProjectPathSchema.shape, async ({ project_path }) => {
    ...
    const text = withRecentActivity(project_path, lines.join("\n"), "ctx_guard", "run paranoid security guard checks");
    return { content: [{ type: "text", text }] };
});
// ── Tool: ctx_grep ──────────────────────────────────────────────────
server.tool("ctx_grep", "Fast text search across the repository using ctx-agent built-in grep (ripgrep-style in Rust). Useful when a symbol query misses strings, routes, handlers, or comments.", {
    ...ProjectPathSchema.shape,
